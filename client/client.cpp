...
void setup() {
  ...
  tft.begin(ID);
  tft.setRotation(1);
  shared.tft->fillScreen(
      TFT_BLUE);  // so we know the map initially draws properly
  ...
  shared.joy_centre = xy_pos(analogRead(clientpins::joy_x_pin),
                             analogRead(clientpins::joy_y_pin));
  ...
  if (!SD.begin(clientpins::sd_cs)) {
    ...
    Serial.println(
        "* Is the chipSelect pin the one for your shield or module?");
...
void process_input() {
  ...
  if (delta.x != 0 || delta.y != 0) {
    ...
    move_cursor(
        delta);  // move the cursor, and the map view if the edge was nudged
