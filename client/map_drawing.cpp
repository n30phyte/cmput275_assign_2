...
void initialize_display_values() {
  ...
  shared.map_coords = xy_pos(mapdata::map_x_limit[shared.map_number] / 2 -
                                 displayconsts::display_width / 2,
                             mapdata::map_y_limit[shared.map_number] / 2 -
                                 displayconsts::display_height / 2);
...
void centre_cursor() {
  shared.cursor_map =
      xy_pos(shared.map_coords.x + displayconsts::display_width / 2,
...
xy_pos get_cursor_screen() {
  return xy_pos(shared.cursor_map.x - shared.map_coords.x,
                shared.cursor_map.y - shared.map_coords.y);
}

// draw the current map view!
void draw_map() {
  lcd_image_draw(&mapdata::map_tiles[shared.map_number], shared.tft,
                 shared.map_coords.x, shared.map_coords.y, 0, 0,
...
void erase_cursor() {
  ...
  lcd_image_draw(&mapdata::map_tiles[shared.map_number], shared.tft,
                 shared.cursor_map.x - mapdata::dot_radius,
                 shared.cursor_map.y - mapdata::dot_radius,
                 cursor_disp.x - mapdata::dot_radius,
                 cursor_disp.y - mapdata::dot_radius,
...
void draw_cursor() {
  xy_pos cursor_disp = get_cursor_screen();
  shared.tft->fillCircle(cursor_disp.x, cursor_disp.y, mapdata::dot_radius,
                         TFT_RED);
}
...
void status_message(const char* msg) {
  shared.tft->fillRect(0, displayconsts::display_height,
                       displayconsts::display_width, 24, TFT_GREEN);
...
void zoom_map(int16_t change) {
  ...
  lon_lat_32 geo(x_to_longitude(shared.map_number, shared.cursor_map.x),
                 y_to_latitude(shared.map_number, shared.cursor_map.y));

  shared.cursor_map =
      xy_pos(longitude_to_x(new_map, geo.lon), latitude_to_y(new_map, geo.lat));
...
void move_cursor(const xy_pos& delta) {
  ...
  if (cur_pos.x < displayconsts::min_x)
    shift.x = -1;
  else if (cur_pos.x > displayconsts::max_x)
    shift.x = 1;

  if (cur_pos.y < displayconsts::min_y)
    shift.y = -1;
  else if (cur_pos.y > displayconsts::max_y)
    shift.y = 1;
